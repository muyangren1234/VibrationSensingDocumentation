...
def decode_data(data_path):
    data = ''
    dataAll = []
    read_path = data_path
    write_path = './decode_data/'
    ...
    for (dir, dirs, files) in os.walk(data_path):
        ...
        for file in files:
            ...
            if fnmatch.fnmatch(file, '*.txt'):
                try:
                    inputStr = read_path + file
                    outStr1 = write_path + file + 'out.txt'
                    f = open(inputStr,'rb')
                    f1 = open(outStr1,'w')

                    dataFile = f.read()
                    data = data + dataFile
                    result = data.find("\xff")
                    print result
